...
import com.juliasoft.pinger.tests.Tools.TestPing
...
class ReachableFutureSpec extends FunSuite with StrictLogging {
  ...
  test("case 1: ReachableFuture should successfully resolve localhost") {
    val p = new TestPing(0, 100, "") with ReachableFuture
  ...
  test("case 3: ReachableFuture should fail on wrong IP") {
    val p = new TestPing(0, 100, "Unreachable host 0.0.0.1") with ReachableFuture
  ...
  test("case 4: ReachableFuture should fail on resolving google")  {
    intercept[java.util.concurrent.TimeoutException] {
      val p = new TestPing(0, 1000, "Unreachable host http://google.com") with ReachableFuture
